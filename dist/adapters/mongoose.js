...
module.exports = function (model, opts) {
  ...
  function detectReference(opts) {
    if (opts.options) {
      ...
      } else if (_.isArray(opts.options.type) && opts.options.type.length && opts.options.type[0].ref && opts.options.type[0].type) {
  ...
  function getTypeFromMongoose(opts) {
    if (_.isPlainObject(opts) && !opts.path) {
      ...
      if (opts.caster.instance && (opts.caster.options.ref || _.keys(opts.caster.options).length === 0)) {
      ...
      return opts.instance || opts.options && getTypeFromNative(opts.options.type) || null;
    }
  }
  ...
  function getFieldSchema(path) {
    ...
    if (opts.options && !_.isNull(opts.options.default) && !_.isUndefined(opts.options.default) && !_.isFunction(opts.options.default)) {
