...
function ResourcesGetter(model, opts, params) {
  ...
  function refilterBasedOnFilters(records) {
    return P.filter(records, function (record) {
      var ret = true; // Refilter record based on the populated match.
    ...
    _.each(params.filter, function (values, key) {
      if (key.indexOf(':') > -1) {
        ...
        if (fieldName === field.field) {
          var currentField = _.find(schema.fields, {
            field: fieldName
          });
          ...
          if (currentField && currentField.reference) {
            ...
            values.split(',').forEach(function (value) {
              var condition = {};
              condition[subFieldName] = new OperatorValueParser(opts, params.timezone).perform(subModel, subFieldName, value);
  ...
  function handleFilterParams(query) {
    ...
    _.each(params.filter, function (values, key) {
      var conditions = new FilterParser(model, opts, params.timezone).perform(key, values);
  ...
  function exec(query) {
    return new P(function (resolve, reject) {
      ...
      if (!hasRelationshipFilter()) {
        query.limit(getLimit()).skip(getSkip());
      }
  ...
  this.perform = function () {
    return getSegmentCondition().then(function () {
      ...
      if (hasRelationshipFilter()) {
        return exec(query).then(function (records) {
