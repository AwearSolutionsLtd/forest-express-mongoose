...
function LineStatFinder(model, params, opts) {
  ...
  function handlePopulate(records, referenceField) {
    return new P(function (resolve, reject) {
      var referenceModel = SchemaUtils.getReferenceModel(opts, referenceField.reference);
  ...
  function formatLabel(record, momentRange) {
    switch (momentRange) {
      case 'day':
        return moment().year(record._id.year).month(record._id.month - 1).startOf('month').add(record._id.day - 1, 'days').startOf(momentRange).format(getFormat(momentRange));

      case 'week':
        return moment().year(record._id.year).week(record._id.week).startOf(momentRange).format(getFormat(momentRange));

      case 'month':
        return moment().year(record._id.year).month(record._id.month - 1).startOf(momentRange).format(getFormat(momentRange));
  ...
  function setDate(record, momentRange) {
    switch (momentRange) {
      case 'day':
        return moment().year(record._id.year).month(record._id.month - 1).startOf('month').add(record._id.day - 1, 'days').startOf(momentRange);

      case 'week':
        return moment().year(record._id.year).week(record._id.week).startOf(momentRange);

      case 'month':
        return moment().year(record._id.year).month(record._id.month - 1).startOf(momentRange);
  ...
  function fillEmptyIntervals(records, momentRange, firstDate, lastDate) {
    ...
    while (currentDate <= lastDate) {
      ...
      var currentRecord = _.find(records, {
        label: currentLabel
      });
    ...
    return new P(function (resolve, reject) {
      ...
      if (params.filterType && params.filters) {
        ...
        _.each(params.filters, function (filter) {
          var conditions = new FilterParser(model, opts, params.timezone).perform(filter.field, filter.value);
